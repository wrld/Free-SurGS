...
def normalize_tensor_image(tensor_image):
    # Ensure the tensor is a float type
    ...
    normalized_image = (tensor_image - min_val) / (max_val - min_val)
...
def visualize_torch_image(tensor_image, name):
    # If the tensor image has 3 channels, we assume it's in the format [C, H, W]
    ...
    # If the tensor image has 4 channels (batch of images), visualize the first one
    elif tensor_image.dim() == 4:
        to_pil = transforms.ToPILImage()
        image = to_pil(tensor_image[0])
        plt.savefig(name)
        # Plot the image
        # plt.imshow(image)
        # plt.axis('off')  # No axes for images
        ...
        print("Unsupported tensor shape for visualization.")
